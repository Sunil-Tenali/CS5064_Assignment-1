...
// GLuint gridVbo, gridVao, cubeVbo, cubeVao, filledCubeVbo, filledCubeVao;
...
//     shaderList.push_back(glFramework::LoadShaderGL(GL_VERTEX_SHADER, "simple_vs.glsl"));
//     shaderList.push_back(glFramework::LoadShaderGL(GL_FRAGMENT_SHADER, "simple_fs.glsl"));
//     shaderProgram = glFramework::CreateProgramGL(shaderList);
// }

// // void initGrid() {
// //     int gridSize = 5; // Grid spans from -2 to +2 in all directions
// //     float gridStep = 1.0f;

// //     // Lines parallel to X-axis
// //     for (int y = -gridSize / 2; y <= gridSize / 2; ++y) {
// //         for (int z = -gridSize / 2; z <= gridSize / 2; ++z) {
// //             gridVertices.push_back(-gridSize / 2); gridVertices.push_back(y); gridVertices.push_back(z); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
// //             gridVertices.push_back(gridSize / 2); gridVertices.push_back(y); gridVertices.push_back(z); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
// //         }
// //     }
...
// //         for (int z = -gridSize / 2; z <= gridSize / 2; ++z) {
// //             gridVertices.push_back(x); gridVertices.push_back(-gridSize / 2); gridVertices.push_back(z); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
// //             gridVertices.push_back(x); gridVertices.push_back(gridSize / 2); gridVertices.push_back(z); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
// //         }
// //     }
...
// //     for (int x = -gridSize / 2; x <= gridSize / 2; ++x) {
// //         for (int y = -gridSize / 2; y <= gridSize / 2; ++y) {
// //             gridVertices.push_back(x); gridVertices.push_back(y); gridVertices.push_back(-gridSize / 2); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
// //             gridVertices.push_back(x); gridVertices.push_back(y); gridVertices.push_back(gridSize / 2); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
// //         }
// //     }
...
// // }
// void initGrid() {
//     int gridDivisions = 5;  // We want 5 divisions
//     float gridStep = 1.0f;
//     float halfGridSize = gridDivisions / 2.0f; // Extends from -2.5 to 2.5

//     // Lines parallel to X-axis (constant Y and Z)
//     for (float y = -halfGridSize; y <= halfGridSize; y += gridStep) {
//         for (float z = -halfGridSize; z <= halfGridSize; z += gridStep) {
//             gridVertices.push_back(-halfGridSize); gridVertices.push_back(y); gridVertices.push_back(z);
...
//     // Lines parallel to Y-axis (constant X and Z)
//     for (float x = -halfGridSize; x <= halfGridSize; x += gridStep) {
//         for (float z = -halfGridSize; z <= halfGridSize; z += gridStep) {
//             gridVertices.push_back(x); gridVertices.push_back(-halfGridSize); gridVertices.push_back(z);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
...
//     // Lines parallel to Z-axis (constant X and Y)
//     for (float x = -halfGridSize; x <= halfGridSize; x += gridStep) {
//         for (float y = -halfGridSize; y <= halfGridSize; y += gridStep) {
//             gridVertices.push_back(x); gridVertices.push_back(y); gridVertices.push_back(-halfGridSize);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
...
//         glm::mat4 cellModel = glm::translate(glm::mat4(1.0f), position);

//         // Apply the same rotation as the grid
//         cellModel = glm::rotate(cellModel, glm::radians(rotationX), glm::vec3(1.0f, 0.0f, 0.0f));
//         cellModel = glm::rotate(cellModel, glm::radians(rotationY), glm::vec3(0.0f, 1.0f, 0.0f));
...
// //     // Snap cube position to nearest integer values (align with grid units)
// //     cubePosition.x = round(cubePosition.x);
// //     cubePosition.y = round(cubePosition.y);
// //     cubePosition.z = round(cubePosition.z);

// //     // Ensure the cube stays within grid boundaries (-2 to +2)
// //     cubePosition.x = std::max(-2.0f, std::min(2.0f, cubePosition.x));
// //     cubePosition.y = std::max(-2.0f, std::min(2.0f, cubePosition.y));
// //     cubePosition.z = std::max(-2.0f, std::min(2.0f, cubePosition.z));


// // }

// void clampCubePosition() {
//     // Snap cube position to the nearest grid cell center
//     cubePosition.x = round(cubePosition.x) + 0.5f;
//     cubePosition.y = round(cubePosition.y) + 0.5f;
//     cubePosition.z = round(cubePosition.z) + 0.5f;

//     // Ensure the cube stays within the grid boundaries (-2 to +2)
//     cubePosition.x = std::max(-2.0f, std::min(2.0f, cubePosition.x));
//     cubePosition.y = std::max(-2.0f, std::min(2.0f, cubePosition.y));
//     cubePosition.z = std::max(-2.0f, std::min(2.0f, cubePosition.z));
// }
...
//                     updatedCubeVertices[i * 6 + 3] = r;
//                     updatedCubeVertices[i * 6 + 4] = g;
//                     updatedCubeVertices[i * 6 + 5] = b;
//                 }
...
//         // Snap the cube position to the nearest grid point
//         cubePosition.x = round(cubePosition.x);
//         cubePosition.y = round(cubePosition.y);
//         cubePosition.z = round(cubePosition.z);

//         // Clamp the cube position within the grid boundaries
//         if (cubePosition.x >= -2.0f && cubePosition.x <= 2.0f &&
//             cubePosition.y >= -2.0f && cubePosition.y <= 2.0f &&
//             cubePosition.z >= -2.0f && cubePosition.z <= 2.0f) {
//             clampCubePosition();  // Ensure proper alignment
//         }
//         // clampCubePosition();
//     }
// }
...
//     GLFWwindow* window = glfwCreateWindow(640, 480, "3D Grid with Cube", NULL, NULL);
...
//         glm::mat4 view_matrix = glm::lookAt(glm::vec3(5.0, 5.0, 5.0), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0));
//         glm::mat4 projection_matrix = glm::perspective(glm::radians(45.0f), 640.0f / 480.0f, 0.1f, 100.0f);
//         model_matrix = glm::rotate(glm::mat4(1.0f), glm::radians(rotationX), glm::vec3(1.0f, 0.0f, 0.0f));
...
//         // Render the cube at its snapped position
//         glm::mat4 cubeModel = glm::translate(model_matrix, cubePosition);
...
// void initGrid() {
//     int gridSize = 5; // 5×5×5 grid
//     float gridSpacing = 1.0f;
//     float startPos = -gridSize / 2.0f;
//     float endPos = gridSize / 2.0f;

//     // Lines parallel to X-axis
//     for (int y = 0; y <= gridSize; ++y) {
//         for (int z = 0; z <= gridSize; ++z) {
//             float yPos = startPos + y * gridSpacing;
//             float zPos = startPos + z * gridSpacing;
            
//             gridVertices.push_back(startPos); gridVertices.push_back(yPos); gridVertices.push_back(zPos);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
            
//             gridVertices.push_back(endPos); gridVertices.push_back(yPos); gridVertices.push_back(zPos);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
//         }
//     }
...
//         for (int z = 0; z <= gridSize; ++z) {
//             float xPos = startPos + x * gridSpacing;
//             float zPos = startPos + z * gridSpacing;
            
//             gridVertices.push_back(xPos); gridVertices.push_back(startPos); gridVertices.push_back(zPos);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
            
//             gridVertices.push_back(xPos); gridVertices.push_back(endPos); gridVertices.push_back(zPos);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
//         }
//     }
...
//             gridVertices.push_back(xPos); gridVertices.push_back(yPos); gridVertices.push_back(startPos);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
            
//             gridVertices.push_back(xPos); gridVertices.push_back(yPos); gridVertices.push_back(endPos);
//             gridVertices.push_back(0.5f); gridVertices.push_back(0.5f); gridVertices.push_back(0.5f);
//         }
//     }
...
// void clampCubePosition() {
//     int gridSize = 5;
//     float minPos = -gridSize / 2.0f;
//     float maxPos = gridSize / 2.0f - 1.0f;
...
//     // Simple vertex shader for the fragment shader to use object color
